...
export default function Ecommerce() {
  return (
    <section className="bg-gradient-to-br from-[#eef6ff] via-[#f8fbff] to-[#e9f1ff] py-20 lg:py-24" id="ecommerce">
      <div className="mx-auto grid w-[min(1180px,calc(100%-40px))] items-center gap-10 lg:grid-cols-2">
        ...
        <div className="rounded-[2rem] border border-blue-100 bg-gradient-to-br from-white to-blue-50 p-5 text-slate-950 shadow-2xl shadow-blue-950/10">
          ...
          <div className="grid min-h-80 grid-cols-[110px_1fr] gap-4 rounded-3xl bg-slate-50 p-5 max-sm:grid-cols-1">
            ...
            <div className="grid content-start gap-4"><span className="h-6 w-2/3 rounded-full bg-blue-800" /><div className="grid grid-cols-3 gap-3">{[1,2,3].map(i => <i className="h-20 rounded-2xl bg-blue-100" key={i} />)}</div><div className="grid gap-3 rounded-2xl bg-white p-4">{[1,2,3,4].map(i => <span className="h-3 rounded-full bg-slate-300" key={i} />)}</div></div>
          </div>
        </div>
      </div>
